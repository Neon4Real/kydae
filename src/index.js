...
app.get('/', (req, res) => {
  res.send('<a href="http://kydae-web.averageneon.repl.co/</a>')
})
...
axios.get('http://kydae-web.averageneon.repl.co/').then(res => {
  if (res.data.tag_name !== version) {
    console.log(chalk.red.bgGreenBright(`Your bot is up to date! !`, version + ' -> ' + res.data.tag_name));
  }
}).catch(err => {
...
console.clear();
console.log(chalk.blue(chalk.bold(`Kydae`)), (chalk.white(`>>`)), (chalk.green(`Starting up`)), (chalk.white(`...`)))
console.log(`\u001b[0m`)
console.log(chalk.red(`© Cmxn3on | 2023 - ${new Date().getFullYear()}`))
console.log(chalk.red(`All rights reserved`))
console.log(`\u001b[0m`)
console.log(`\u001b[0m`)
console.log(chalk.blue(chalk.bold(`kydae`)), (chalk.white(`>>`)), chalk.red(`Version ${require(`${process.cwd()}/package.json`).version}`), (chalk.green(`loaded`)))
console.log(`\u001b[0m`);

manager.on('shardCreate', shard => {
  let embed = new Discord.EmbedBuilder()
    .setTitle(`🆙・Launching console`)
    .setDescription(`Connected To Database Server`)
    .setFields([
      {
        name: "🆔┆ID",
  ...
  console.log(chalk.blue(chalk.bold(`kydae`)), (chalk.white(`>>`)), (chalk.green(`Starting`)), chalk.red(`Shard #${shard.id + 1}`), (chalk.white(`...`)))
  console.log(`\u001b[0m`);
